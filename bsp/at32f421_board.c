...
char version[]="1.3";
char model[] = "IOEXT_AT32F421";
char date[] = "2023.03.27";

void print_welcome(){
	cust_printf("%s, Firmware v%s\r\n", model, version);
	cust_printf("%s\r\n", date);
	cust_printf("AT32F421\r\n");
}
...
//  at32_button_init();
  ...
  if(crm_flag_get(CRM_WDT_RESET_FLAG) != RESET)
  {
    ...
    cust_printf("rst from wdt\r\n");
  }
  else
  {
    /* reset from other mode */
    cust_printf("rst normal\r\n");
  }
